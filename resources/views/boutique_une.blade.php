...
<div class="modal fade" id="Modal_image" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" >
    <div class="modal-dialog modal-dialog-centered" role="document" >
      <div class="modal-content" style="background-color: black">
        ...
        <div class="modal-body">
            ...
            <div style="padding-bottom: 10px">            <img id="img_user" style="border-radius: 50%;background-color:white" src={{ asset('storage/user.png') }} alt="" height="30" width="30"><div id="name_user" style="display: inline;padding-left:5px;color:white">test</div>
        </div>
            <div style="text-align: center"><img onclick="detail_produit()" id="image_popup" src="{{ asset('storage/A_black_image.jpg') }}" class="img_stor"> <div onclick="right()" class="div_right"></div> <div onclick="left()" class="div_left"> </div></div>
  ...
  @if(App::getlocale()=="ar")
  <h1  style="padding: 20px;color:#263066;text-align:end">{{__('boutique_une.titre')}}</h1>
  @else
  <h1  style="padding: 20px;color:#263066;text-align:start">{{__('boutique_une.titre')}}</h1>
  @endif
...
<div class=" containe mt-2 ">
  ...
  <div class="scrollable-tabs-container container">
    ...
    <ul >
        ...
        $name='"'.$boutique['nom'].'"';
        @endphp
      <li class="text-center" @if($boutique['story']=='1') onclick="show_image({{$boutique['id_utilisateur'].','.$btq_image.','.$name}})" @else onclick="get_prod_user({{$boutique['id_utilisateur']}})" @endif >
