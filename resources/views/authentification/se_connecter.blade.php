...
<div class="container mt-3">
    ...
    <form action={{route('seconnecter')}} method="POST">
      ...
      <div class="mb-3 mt-3" @if(App::getlocale()=="ar") style="text-align: end" @endif>
        ...
        <input @if(App::getlocale()=="ar") style="text-align: end" @endif type="email" class="form-control border-top-0 border-right-0 border-left-0  @error('email') is-invalid @enderror" id="email" placeholder="{{__('login.entreemail')}}" name="email" value="{{ old('email') }}" required>
        @error('email')
      ...
      <div class="mb-3" @if(App::getlocale()=="ar") style="text-align: end" @endif>
        ...
        <input @if(App::getlocale()=="ar") style="text-align: end" @endif type="password" class="form-control border-top-0 border-right-0 border-left-0 @error('pswd') is-invalid @enderror" id="pwd" placeholder="{{__('login.entrepassword')}}" name="pswd" value="{{ old('pswd') }}" required>
        @error('pswd')
        <div class="invalid-feedback">{{$message}}</div>
        @enderror
      </div>
